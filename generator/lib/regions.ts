...
export async function getAllRegions(): Promise<string[]> {
    ...
    regions.push(
      ...page.Regions!.map((r) => {
        if (r.RegionOptStatus == 'DISABLED') {
          const enableCommand = new EnableRegionCommand({
  ...
  const listCommand = new ListRegionsCommand({
    RegionOptStatusContains: [RegionOptStatus.ENABLING],
  });
  for (
    let output = await accountClient.send(listCommand);
    output.Regions!.length > 0;
    output = await accountClient.send(listCommand)
  ) {
    console.log('Regions still being enabled: ', output.Regions);
    await new Promise((r) => setTimeout(r, 5000));
  }
  writeDataToFile('src/lib/regions.json', regions);
  return regions;
}
